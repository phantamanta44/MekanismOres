...
public class Gas
{
	...
	private static Method getFluidTexture;
	private static Object fluidTypeStill;
	private static boolean triedToResolve = false;
	...
	public TextureAtlasSprite getSprite()
	{
		if (from_fluid) {
			if (!triedToResolve) {
				triedToResolve = true;
				try {
					getFluidTexture = Class.forName("mekanism.client.render.MekanismRenderer")
							.getDeclaredMethod("getFluidTexture");
					fluidTypeStill = Class.forName("mekanism.client.render.MekanismRenderer.FluidType")
							.getDeclaredField("STILL");
				} catch (Exception e) {
					return sprite;
				}
			}
			if (getFluidTexture == null || fluidTypeStill == null)
				return sprite;
			try {
				return (TextureAtlasSprite)getFluidTexture.invoke(null, fluid, fluidTypeStill);
			} catch (Exception e) {
				return sprite;
			}
		}
	 ...
	 * @param map - Texture map to register the icon with.
	 * @return this Gas object
	 */
